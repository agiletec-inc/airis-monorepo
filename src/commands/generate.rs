...
fn generate_cargo_toml(manifest: &Manifest, engine: &TemplateEngine) -> Result<()> {
    let cargo_path = Path::new("Cargo.toml");

    // Check if existing Cargo.toml is auto-generated (safe to overwrite)
    if cargo_path.exists() {
        let existing = fs::read_to_string(cargo_path).unwrap_or_default();
        if !existing.starts_with("# Auto-generated by airis") {
            println!("{}", "⚠️  Skipping Cargo.toml: not auto-generated (manual edit detected)".yellow());
            println!("   Add '# Auto-generated by airis init' at the top to enable auto-generation");
            return Ok(());
        }
    }

    let content = engine.render_cargo_toml(manifest)?;
