...
impl TemplateEngine {
    ...
    /// Generate CLAUDE.md from manifest.toml
    /// This provides Claude Code with all necessary context for the project
    pub fn render_claude_md(&self, manifest: &Manifest) -> Result<String> {
        let mut sections = vec![];

        // Header
        sections.push(
            "# CLAUDE.md\n\n\
             > Auto-generated by airis init. Customize as needed."
                .to_string(),
        );

        // Project Overview
        let mode_str = match manifest.mode {
            Mode::DockerFirst => "docker-first",
            Mode::Hybrid => "hybrid",
            Mode::Strict => "strict",
        };
        sections.push(format!(
            "## Project Overview\n\n\
             - **Name**: {}\n\
             - **Mode**: {} (docker-first / hybrid / strict)\n\
             - **Package Manager**: {}",
            manifest.workspace.name, mode_str, manifest.workspace.package_manager
        ));

        // Docker First Development section (guards)
        if !manifest.guards.deny.is_empty() || !manifest.guards.deny_with_message.is_empty() {
            let mut guard_lines = vec!["## Docker First Development\n".to_string()];
            guard_lines.push("### Forbidden → Alternative Commands\n".to_string());
            guard_lines.push("| Forbidden | Alternative |".to_string());
            guard_lines.push("|-----------|-------------|".to_string());

            // Add deny commands
            for cmd in &manifest.guards.deny {
                let alt = manifest.remap.get(cmd).map(|s| format!("`{}`", s)).unwrap_or_else(|| "Use airis CLI".to_string());
                guard_lines.push(format!("| `{}` | {} |", cmd, alt));
            }

            // Add deny_with_message commands
            for (cmd, msg) in &manifest.guards.deny_with_message {
                guard_lines.push(format!("| `{}` | {} |", cmd, msg));
            }
        ...
        if !manifest.commands.is_empty() {
            ...
            for (name, cmd) in &manifest.commands {
                cmd_lines.push(format!("| `airis {}` | `{}` |", name, cmd));
            }
        ...
        sections.push(format!(
            ...
            manifest.ci.auto_merge.to, manifest.ci.auto_merge.from
        ));

        // Workspace Structure
        if !manifest.packages.workspaces.is_empty() {
            let mut ws_lines = vec!["## Workspace Structure\n".to_string()];
            ws_lines.push("```".to_string());
            for ws in &manifest.packages.workspaces {
                ws_lines.push(format!("{}/", ws));
            }
            ws_lines.push("```".to_string());
            sections.push(ws_lines.join("\n"));
        }
        ...
        if !manifest.apps.is_empty() {
            ...
            for (name, app) in &manifest.apps {
                let app_type = app.app_type.as_deref().unwrap_or("unknown");
                app_lines.push(format!("- **{}** ({})", name, app_type));
            }
        ...
        sections.push(
            "---\n\n\
             *Generated by airis-monorepo. See manifest.toml for full configuration.*"
                .to_string(),
        );
...
"docker compose up" = "Use `airis up` instead"
...
"#;
        let manifest: Manifest = toml::from_str(toml_str).unwrap();
        let engine = TemplateEngine::new().unwrap();
        let result = engine.render_claude_md(&manifest).unwrap();
        ...
        // Should contain project overview
        assert!(result.contains("**Name**: my-project"));
        assert!(result.contains("**Mode**: docker-first"));
        assert!(result.contains("**Package Manager**: pnpm@10.22.0"));

        // Should contain guards table
        assert!(result.contains("## Docker First Development"));
        assert!(result.contains("### Forbidden → Alternative Commands"));
        assert!(result.contains("| `npm` |"));
        assert!(result.contains("| `yarn` |"));
        assert!(result.contains("| `docker compose up` | Use `airis up` instead |"));

        // Should contain remap table
        assert!(result.contains("### Command Remapping"));
        assert!(result.contains("| `npm install` | `airis install` |"));
        assert!(result.contains("| `pnpm install` | `airis install` |"));

        // Should contain commands table
        assert!(result.contains("## Available Commands"));
        assert!(result.contains("| `airis dev` | `pnpm dev` |"));
        assert!(result.contains("| `airis up` | `docker compose up -d` |"));

        // Should contain git workflow
        assert!(result.contains("## Git Workflow"));
        assert!(result.contains("**Main branch**: main"));
        assert!(result.contains("**Dev branch**: dev"));

        // Should contain workspace structure
        assert!(result.contains("## Workspace Structure"));
        assert!(result.contains("apps/*/"));
        assert!(result.contains("libs/*/"));
    }
...
"#;
    ...
    #[test]
    fn test_render_claude_md_with_apps_and_libs() {
        let toml_str = r#"
[workspace]
name = "monorepo"
service = "workspace"
...
"#;
        ...
        // Should contain apps section
        assert!(result.contains("## Applications"));
        assert!(result.contains("**dashboard** (nextjs)"));
        assert!(result.contains("**api** (hono)"));

        // Should contain libs section
