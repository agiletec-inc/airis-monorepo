...
impl TemplateEngine {
    ...
    pub fn render_cargo_toml(&self, manifest: &Manifest) -> Result<String> {
        let data = self.prepare_cargo_toml_data(manifest)?;
        self.hbs
    ...
    fn prepare_cargo_toml_data(&self, manifest: &Manifest) -> Result<serde_json::Value> {
        let binary_name = if manifest.project.binary_name.is_empty() {
            manifest.project.id.clone()
        } else {
            manifest.project.binary_name.clone()
        ...
        let edition = if manifest.project.rust_edition.is_empty() {
            "2024".to_string()
        } else {
            manifest.project.rust_edition.clone()
        };

        // Format authors for Cargo.toml
        let authors: Vec<String> = manifest.project.authors.clone();

        // Format keywords for Cargo.toml
        let keywords: Vec<String> = manifest.project.keywords.clone();

        // Format categories for Cargo.toml
        let categories: Vec<String> = manifest.project.categories.clone();
...
const CARGO_TOML_TEMPLATE: &str = r#"# Auto-generated by airis init
# DO NOT EDIT - change manifest.toml instead.
...
# Dependencies are managed manually in this file for now.
# Future: will be generated from manifest.toml [rust.dependencies]
[dependencies]
